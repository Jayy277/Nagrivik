...
export async function POST(
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/admin/duplicates/scan/${issueId}`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Error scanning issue for duplicates' }, { status: res.status });
    }
