...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/admin/duplicates/suggestions?${searchParams.toString()}`, {
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Error fetching duplicate suggestions' }, { status: res.status });
    }
