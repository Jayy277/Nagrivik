...
export async function POST(
  ...
  try {
    ...
    const reason = body?.reason || 'Violates community moderation policy';
    const notes = body?.notes || '';

    const query = `?reason=${encodeURIComponent(reason)}${notes ? `&notes=${encodeURIComponent(notes)}` : ''}`;
    const res = await fetch(`${BACKEND_URL}/api/moderation/content/${targetType}/${targetId}/hide${query}`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Unable to hide content' }, { status: res.status });
    }
