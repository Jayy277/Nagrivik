...
export async function POST(
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/moderation/reports/${id}/review`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Unable to review report' }, { status: res.status });
    }
