...
export async function POST(
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/moderation/reports/${id}/dismiss${query}`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Unable to dismiss report' }, { status: res.status });
    }
