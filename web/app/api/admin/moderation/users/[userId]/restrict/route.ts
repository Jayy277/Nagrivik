...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/moderation/users/${userId}/restrict`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ message: err.message || 'Unable to restrict user' }, { status: res.status });
    }
