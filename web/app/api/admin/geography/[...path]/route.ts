...
async function handleProxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  const url = `${BACKEND_URL}/api/admin/geography/${targetPath}${queryString}`;
  ...
  try {
    ...
    return NextResponse.json({ message: 'Unable to connect to Nagrivic services', error: String(error) }, { status: 500 });
  }
}
